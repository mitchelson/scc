...
class MilitarController {
  async index(request, response) {
    //Listar todos os militares sem nenhum parametro
    try {
  ...
  async show(request, response) {
    //Pesquisar militar especifico passando id do militar como parametro
    try {
  ...
  async destroy(request, response) {
    //Deletar registro específico, necessita de parametro id
    try {
      ...
      return response
        .status(400)
        .json({ error: "Erro ao Deletar Registro do Militar" });
    }
  }
}
