...
class ViaturaController {
  async index(request, response) {
    //Listar todos os viaturaes sem nenhum parametro
    try {
  ...
  async show(request, response) {
    //Pesquisar viatura especifico passando id do viatura como parametro
    try {
  ...
  async destroy(request, response) {
    ...
    try {
      ...
      return response
        .status(400)
        .json({ error: "Erro ao Deletar Registro do Viatura" });
    }
  }
}
